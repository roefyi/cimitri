...
export default async function CrewTodayPage({
  ...
  const [jobs, monthJobs] = await Promise.all([
    listJobsInRange(session.shopId, listBounds.from, listBounds.to, session.personId),
    listJobsInRange(session.shopId, month.from, month.to, session.personId),
  ]);
  ...
  return (
    <div className='flex flex-col gap-6'>
      <PageHeader
        ...
        description={
          range === 'week'
            ? `Week of ${formatLongDate(week.from)} · ${session.personName}`
      ...
      <div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
        ...
        <StatCard
          label='Flagged'
          value={flagged}
          hint={flagged ? 'Office will see this' : undefined}
        />
      </div>
      ...
      <div className='grid items-start gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]'>
        <PageCard title='Calendar'>
          <div className='mb-4 flex rounded-10 bg-bg-weak-50 p-1'>
            <Link
              href={`/crew?range=today&date=${date}`}
              className={cn(
            ...
            <Link
              href={`/crew?range=week&date=${date}`}
              className={cn(
                'flex-1 rounded-md px-3 py-1.5 text-center text-label-sm',
                range === 'week'
                  ? 'bg-bg-white-0 text-text-strong-950 shadow-toggle-switch'
          ...
          <div className='-mx-2 overflow-x-auto'>
            <ScheduleCalendar
              ...
              hrefForDate={`/crew?range=${range}&date=DATE`}
            />
          </div>
        </PageCard>
          ...
          >
            {jobs.length === 0 ? (
              <p className='px-5 py-8 text-paragraph-sm text-text-sub-600'>
                No jobs assigned to you in this range.
              </p>
            ) : (
              <>
